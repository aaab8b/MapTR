...
class BF16Module(nn.Module):
    ...
    @property
    def weight(self):
        return self.origin_module.weight

    @property
    def bias(self):
        return self.origin_module.bias if hasattr(self.origin_module, 'bias') else None

    def forward(self, x):
        with autocast('cuda', dtype=torch.bfloat16):
            return self.origin_module(x).to(torch.float32)

def convert_conv_linear_to_bf16(module):
    for child_name, child in module.named_children():
        if isinstance(child, (nn.Conv2d, nn.Linear)):
            setattr(module, child_name, BF16Module(child))
        else:
            convert_conv_linear_to_bf16(child)
...
def main():
    ...
    # import pdb
    # pdb.set_trace()
    convert_conv_linear_to_bf16(model)
